...
func NewTicker(apiKey string, accessToken string) *Ticker {
	ticker := &Ticker{
		apiKey:              apiKey,
...
func (t *Ticker) parsePacket(b []byte) (Tick, error) {
	...
	if len(b) == modeQuoteIndexPacketLength || len(b) == modeFullIndexLength {
		...
		if len(b) == modeFullIndexLength {
			tick.Mode = ModeFull
			tick.Timestamp = Time{time.Unix(int64(binary.BigEndian.Uint32(b[28:32])), 0)}
		}
	...
	if len(b) == modeFullLength {
		tick.Mode = ModeFull
		tick.LastTradeTime = Time{time.Unix(int64(binary.BigEndian.Uint32(b[44:48])), 0)}
		...
		tick.Timestamp = Time{time.Unix(int64(binary.BigEndian.Uint32(b[60:64])), 0)}
		...
		for i := 0; i < depthItems; i++ {
			tick.Depth.Buy[i] = DepthItem{
				Quantity: int(binary.BigEndian.Uint32(b[buyPos : buyPos+4])),
				Price:    t.convertPrice(seg, float64(binary.BigEndian.Uint32(b[buyPos+4:buyPos+8]))),
				Orders:   int(binary.BigEndian.Uint16(b[buyPos+8 : buyPos+10])),
			}

			tick.Depth.Sell[i] = DepthItem{
				Quantity: int(binary.BigEndian.Uint32(b[sellPos : sellPos+4])),
				Price:    t.convertPrice(seg, float64(binary.BigEndian.Uint32(b[sellPos+4:sellPos+8]))),
				Orders:   int(binary.BigEndian.Uint16(b[sellPos+8 : sellPos+10])),
			}
